...
class ProductController(
    ...
    fun createProduct(
        ...
        val product = request.toDomain(sellerId)
        ...
        return productService.createProduct(product, sellerId).map { createdProduct ->
            ...
            ResponseEntity.created(location).body("Product created successfully.")
        }
    }
    ...
    @PreAuthorize("hasRole('SELLER')")
    fun updateProduct(
        @PathVariable id: String, @Valid @RequestBody request: UpdateProductRequest, authentication: Authentication
    ): Mono<ResponseEntity<ApiResponse<String>>> {
        return productService.updateProduct(id, request, authentication.name)
    ...
    fun updateProductVisibility(
        @PathVariable id: String, @RequestParam visibility: ProductVisibility, authentication: Authentication
    ): Mono<ResponseEntity<ApiResponse<String>>> {
    ...
    fun createProductsBulk(
        ...
        val sellerId = authentication.name
        val products = requests.map { it.toDomain(sellerId) }
    ...
    fun updateProductsStatusBulk(
        @RequestBody productIds: List<String>, @RequestParam status: ProductStatus, authentication: Authentication
    ): Mono<ResponseEntity<ApiResponse<String>>> {
        val sellerId = authentication.name
