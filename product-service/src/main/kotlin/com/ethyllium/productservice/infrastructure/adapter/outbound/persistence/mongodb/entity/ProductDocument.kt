...
import com.ethyllium.productservice.infrastructure.adapter.inbound.rest.dto.response.*
...
fun ProductDocument.toDomain(): Product = Product(
    ...
    sku = this.sku,
    barcode = this.barcode,
    // Create placeholder objects with only the ID.
...
data class ProductDocumentFile(
    ...
    fun toResponse() = ProductDocumentResponse(
        url = url, name = name, type = type, size = size, mimeType = mimeType
    )
}
...
fun Product.toDocument(): ProductDocument = ProductDocument(id = this.id?.let { ObjectId(it) } ?: ObjectId.get(),
    name = this.name,
    description = this.description,
    shortDescription = this.shortDescription,
    sku = this.sku,
